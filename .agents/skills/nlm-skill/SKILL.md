...
3. **âš ï¸ ALWAYS ASK USER BEFORE DELETE**: Before executing ANY delete command, ask the user for explicit confirmation. Deletions are **irreversible**. Show what will be deleted and warn about permanent data loss.
...
10. **Choose output format wisely**: Default output (no flags) is compact and token-efficientâ€”use it for status checks. Use `--quiet` to capture IDs for piping. Only use `--json` when you need to parse specific fields programmatically.
11. **Use `--help` when unsure**: Run `nlm <command> --help` to see available options and flags for any command.

## Codex Sandbox Networking

When using the nlm CLI from Codex, first verify whether the shell has outbound HTTPS. In sandboxed `workspace-write` sessions, network access may be restricted even though Codex web search works through a separate tool. A quick test is:

```powershell
Invoke-WebRequest -Uri "https://www.google.com" -UseBasicParsing -TimeoutSec 10 | Select-Object StatusCode
```

If sandboxed HTTPS fails, rerun NLM commands with escalated/out-of-sandbox execution rather than debugging NotebookLM auth. In Tom's Windows Codex setup, sandboxed `nlm notebook list` failed by trying `127.0.0.1:9`, while escalated execution of this command worked and returned notebooks:

```powershell
$env:PYTHONIOENCODING="utf-8"; & "C:\Users\tomew\.local\bin\nlm.EXE" notebook list
```

Prefer the explicit executable above when PATH may select `C:\Users\tomew\AppData\Local\Programs\Python\Python312\Scripts\nlm.exe` instead.
...
User wants to...
â”‚
â”œâ”€â–º Work with NotebookLM for the first time
â”‚   â””â”€â–º nlm login â†’ nlm notebook create "Title"
â”‚
â”œâ”€â–º Add content to a notebook
â”‚   â”œâ”€â–º From a URL/webpage â†’ nlm source add <nb-id> --url "https://..."
â”‚   â”œâ”€â–º From YouTube â†’ nlm source add <nb-id> --url "https://youtube.com/..."
â”‚   â”œâ”€â–º From pasted text â†’ nlm source add <nb-id> --text "content" --title "Title"
â”‚   â”œâ”€â–º From Google Drive â†’ nlm source add <nb-id> --drive <doc-id> --type doc
â”‚   â””â”€â–º Discover new sources â†’ nlm research start "query" --notebook-id <nb-id>
â”‚
â”œâ”€â–º Generate content from sources
â”‚   â”œâ”€â–º Podcast/Audio â†’ nlm audio create <nb-id> --confirm
â”‚   â”œâ”€â–º Written summary â†’ nlm report create <nb-id> --confirm
â”‚   â”œâ”€â–º Study materials â†’ nlm quiz/flashcards create <nb-id> --confirm
â”‚   â”œâ”€â–º Visual content â†’ nlm mindmap/slides/infographic create <nb-id> --confirm
â”‚   â”œâ”€â–º Video â†’ nlm video create <nb-id> --confirm
â”‚   â””â”€â–º Extract data â†’ nlm data-table create <nb-id> "description" --confirm
â”‚
â”œâ”€â–º Ask questions about sources
â”‚   â””â”€â–º nlm notebook query <nb-id> "question"
â”‚       (Use --conversation-id for follow-ups)
â”‚       âš ï¸ Do NOT use `nlm chat start` - it's a REPL for humans only
â”‚
â”œâ”€â–º Check generation status
â”‚   â””â”€â–º nlm studio status <nb-id>
â”‚
â””â”€â–º Manage/cleanup
    â”œâ”€â–º List notebooks â†’ nlm notebook list
    â”œâ”€â–º List sources â†’ nlm source list <nb-id>
    â”œâ”€â–º Delete source â†’ nlm source delete <source-id> --confirm
    â””â”€â–º Delete notebook â†’ nlm notebook delete <nb-id> --confirm
```
...
Workflow: `research_start` â†’ poll `research_status` â†’ `research_import`
...
- Creates a NEW artifact â€” the original is not modified
...
**Status values**: `completed` (âœ“), `in_progress` (â—), `failed` (âœ—)
...
> âš ï¸ **AI TOOLS: DO NOT USE `nlm chat start`** - It launches an interactive REPL that cannot be controlled programmatically. Use `nlm notebook query` for one-shot Q&A instead.
...
### Pattern 1: Research â†’ Podcast Pipeline
